...
- [work-must-belong-to-a-ticket.md](work-must-belong-to-a-ticket.md) — ticket 归属、无当前 ticket 时的 discover/new/claim 启动流程、`brief` 字段、显示当前 ticket、只开 backlog ticket、自主选择旧票或新票、lifecycle 结果报告和 release handoff 细则；**开始 / 恢复 / ticket 生命周期操作后**
- [workstream-boundaries-must-split-ticket.md](workstream-boundaries-must-split-ticket.md) — 工作流边界变化时判断是否仍是同一 handoff unit；同一 goal / branch 的实现到 PR 默认复用，独立 PR 线、owner、workspace、交付物或长票才拆 ticket / fork；**new repo / new PR line / 页面族 / benchmark / source repo 切换 / 长 ticket**
...
- [published-content-must-identify-ticket.md](published-content-must-identify-ticket.md) — 发布到 GitHub、PR description/comment、文档页面、benchmark 结论或外部报告前，正文必须标注当前 ticket；正式技术结论必须 pin 可追溯 source commit；发布后反向记录 URL；**改已有文档 / 发布 GitHub / PR description / 外部报告 / benchmark 结论前**

### PR review / subagent review

- [pr-review-subagents-must-use-aticket.md](pr-review-subagents-must-use-aticket.md) — PR review subagent 必须接受 sts-harness 控制面上下文，先有 review ticket，搜索历史 ticket 获取背景，先独立发现问题再复核已知问题，在 ticket workspace 里做实验，并把 review 证据、结论和发布物双向链接；**review PR / subagent review / PR comment 前**
...
- [performance-benchmarks-use-perf-run-guard.md](performance-benchmarks-use-perf-run-guard.md) — 性能 benchmark 使用 perf-run-guard 固化 clean CPU/SMT sibling 检查、运行期 CPU 频率采样和 false-green-resistant artifacts；**performance benchmark / perf review / taskset / clean CPU / SMT sibling**
...
| [performance-benchmarks-use-perf-run-guard.md](performance-benchmarks-use-perf-run-guard.md) | 性能 benchmark 的 perf-run-guard 保护 |
...
| [workstream-boundaries-must-split-ticket.md](workstream-boundaries-must-split-ticket.md) | workstream 边界判断和按需拆票 |
